...
export default function BlogArticle() {
  ...
  function renderContent(block: BlogContentBlock, i: number) {
    switch (block.type) {
      ...
      case 'heading':
        return (
          <h2 key={i} className={`text-xl sm:text-2xl font-bold text-white mb-4 mt-10 ${block.level === 2 ? '' : ''}`}>
            {block.text}
          </h2>
        );
      case 'quote':
        return (
          <div key={i} className="relative my-8 pl-6 sm:pl-8 border-l-2 border-accent">
            <p className="text-lg sm:text-xl text-white/90 italic leading-relaxed mb-3">
  ...
  return (
    <main>
      ...
      <nav aria-label="Breadcrumb" className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 sm:pt-28 pb-4">
        ...
        >
          <li><Link to="/" className="hover:text-white transition-colors"><Home className="w-3.5 h-3.5" /></Link></li>
          <li className="flex items-center gap-1.5"><ChevronRight className="w-3 h-3" /><Link to="/blog" className="hover:text-white transition-colors">Blog</Link></li>
          <li className="flex items-center gap-1.5"><ChevronRight className="w-3 h-3" /><span className="text-white">{article.title}</span></li>
        </motion.ol>
      </nav>
      ...
      <section className="relative pb-8 sm:pb-12">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <Link to="/blog" className="inline-flex items-center gap-2 text-sm text-muted hover:text-white transition-colors mb-6">
              ...
              {author && (
                <AnimatedSection delay={0.1}>
                  <div className="glass rounded-2xl p-6 border border-white/[0.06] text-center">
                    ...
                    <h3 className="text-base font-semibold text-white">{author.name}</h3>
              ...
              <AnimatedSection delay={0.3}>
                <div className="glass rounded-2xl p-6 border border-white/[0.06] text-center">
                  ...
                  <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-2">
                    <input type="email" placeholder="Your email" className="h-10 px-4 text-sm bg-white/[0.04] border border-white/[0.10] rounded-full text-white placeholder:text-muted focus:outline-none focus:border-accent/40 transition-colors" required />
                    <button type="submit" className="h-10 text-sm font-medium bg-accent text-black rounded-full hover:shadow-[0_0_25px_rgba(0,229,255,0.3)] transition-all">
                      Subscribe
                    </button>
                  </form>
                </div>
                    ...
                    <Link to={`/blog/${ra.slug}`} className="block h-full group">
                      <article className="glass rounded-2xl overflow-hidden h-full border border-white/[0.06] hover:border-accent/20 transition-all duration-500">
                        <div className="relative aspect-[16/9] overflow-hidden bg-white/[0.02]">
                          ...
                          <span className="absolute top-3 left-3 px-2 py-0.5 rounded-full bg-white/10 backdrop-blur-sm text-[10px] text-white/90">{ra.category}</span>
                        </div>
                        <div className="p-4 sm:p-5">
                          <h3 className="text-base font-semibold text-white mb-2 group-hover:text-accent transition-colors line-clamp-2">{ra.title}</h3>
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <AnimatedSection delay={0.1}>
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
              <a href="#cta" className="px-9 py-4 text-lg font-medium bg-accent text-black rounded-full hover:shadow-[0_0_30px_rgba(0,229,255,0.3)] hover:scale-105 transition-all duration-300 inline-flex items-center gap-2">
                Start a Project <ArrowRight className="w-5 h-5" />
              </a>
              <Link to="/blog" className="px-9 py-4 text-lg font-medium border border-white/20 text-white rounded-full hover:bg-white/5 hover:border-white/40 transition-all duration-300 inline-flex items-center gap-2">
                More Articles
              </Link>
            </div>
          </AnimatedSection>
