...
const columns: Column<Lead>[] = [
  ...
  { key: 'interest', header: 'Interest' },
  {
    ...
    render: (item) => (
      <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${statusColors[item.status] || 'bg-gray-800 text-gray-200'}`}>
        {item.status}
      </span>
    ),
  },
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-200 mt-1">Manage incoming leads and inquiries</p>
        </div>
        <span className="text-sm text-gray-200">{leads.length} total</span>
      </div>
        ...
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-gray-200 text-xs">Name</p>
              ...
              <div>
                <p className="text-gray-200 text-xs">Company</p>
              ...
              <div>
                <p className="text-gray-200 text-xs">Interest</p>
                <p className="font-medium">{selected.interest || '—'}</p>
              </div>
              <div>
                <p className="text-gray-200 text-xs">Source</p>
              ...
              <div>
                ...
                {statusUpdating === selected.id && <span className="text-xs text-gray-200 ml-2">Updating...</span>}
              </div>
            </div>
            {selected.message && (
              <div>
                <p className="text-gray-200 text-xs mb-1">Message</p>
