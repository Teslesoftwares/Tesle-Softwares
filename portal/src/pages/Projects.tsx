...
export default function Projects() {
  ...
  return (
    <div>
      ...
      {projects.length === 0 ? (
        <div className="text-center py-12 text-gray-500">No projects assigned yet.</div>
      ) : (
        <div className="grid gap-4">
          {projects.map((p) => (
            <Link key={p.id} to={`/portal/projects/${p.id}`}
              ...
              {p.deadline && <p className="text-xs text-gray-500 mt-2">Deadline: {new Date(p.deadline).toLocaleDateString()}</p>}
            </Link>
          ))}
        </div>
      )}
