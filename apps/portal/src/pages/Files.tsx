    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-200 mt-1">Shared documents and files</p>
        </div>
      ...
      {showUpload && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-6 space-y-4">
          ...
          <div className="flex justify-end gap-3">
            <button onClick={() => setShowUpload(false)} className="px-4 py-2 text-sm text-gray-200 hover:text-white">Cancel</button>
          ...
          files.map((f) => (
            <div key={f.id} className="flex items-center justify-between px-5 py-4 border-b border-gray-800/50 last:border-0">
              ...
              <a href={f.file_url} target="_blank" rel="noopener noreferrer"
                className="p-2 text-gray-200 hover:text-[#d4a853] transition-colors">
                <Download size={16} />
              </a>
            </div>
          ))
