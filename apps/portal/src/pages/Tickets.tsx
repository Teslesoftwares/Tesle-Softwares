    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-200 mt-1">Get help from our team</p>
        </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        {tickets.length === 0 ? (
          <div className="text-center py-12 text-gray-300">No tickets yet.</div>
        ) : (
          tickets.map((t) => (
            <div key={t.id} onClick={() => openTicket(t)}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-200 mt-0.5 truncate">{t.description}</p>
              </div>
            ...
            <div className="p-4 border-b border-gray-800">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-200 mt-1">{selected.description}</p>
                </div>
                <button onClick={() => setSelected(null)} className="text-gray-200 hover:text-white text-lg">✕</button>
              </div>
            </div>
            <div className="p-4 space-y-3">
              {selected.comments?.map((c: any) => (
                <div key={c.id} className="flex gap-3">
                  <div className={`mt-1 w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0 ${c.is_admin ? 'bg-[#d4a853]/20 text-[#d4a853]' : 'bg-gray-700 text-gray-200'}`}>
                    {c.is_admin ? 'A' : 'C'}
                  </div>
