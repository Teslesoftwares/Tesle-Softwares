...
export function Loader({ loaded, onLoaded }: LoaderProps) {
  ...
  useEffect(() => {
    if (loaded) return;
    const hasSeen = localStorage.getItem('tesle-loader-seen');
    ...
    function draw() {
      ...
      for (let i = 0; i < SEGMENTS; i++) {
        ...
        ctx!.lineWidth = lw;
        ctx!.shadowColor = isLogo ? 'rgba(0, 229, 255, 0.4)' : 'transparent';
      ...
      if (phase <= 4) {
        ...
        for (let i = trail.length - 1; i >= 0; i--) {
          ...
          ctx!.fillStyle = `rgba(0, 229, 255, ${t.a * 0.35})`;
          ctx!.fill();
        }
        ...
        sg.addColorStop(0.3, 'rgba(0, 229, 255, 0.1)');
      ...
      if (phase >= 6) {
        ...
        bg.addColorStop(0, `rgba(0, 229, 255, ${burst * 0.4})`);
        bg.addColorStop(0.1, `rgba(255, 255, 255, ${burst * 0.15})`);
        bg.addColorStop(0.25, `rgba(0, 229, 255, ${burst * 0.06})`);
        bg.addColorStop(0.5, `rgba(0, 229, 255, ${burst * 0.02})`);
