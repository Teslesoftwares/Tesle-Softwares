  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-200 mt-1">Overview of your site</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          <div className="space-y-3">
            {data?.recentLeads.map((lead) => (
              <div key={lead.id} className="flex items-center justify-between py-2 border-b border-gray-800/50 last:border-0">
                <div>
                  ...
                  <p className="text-xs text-gray-200">{lead.email}{lead.company ? ` · ${lead.company}` : ''}</p>
                </div>
            ...
            {!data?.recentLeads.length && <p className="text-sm text-gray-300">No leads yet.</p>}
          </div>
        </div>
            ...
            {data?.recentBlog.map((post) => (
              <div key={post.id} className="flex items-center justify-between py-2 border-b border-gray-800/50 last:border-0">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ml-3 ${
                  post.published ? 'bg-green-900/30 text-green-400' : 'bg-gray-800 text-gray-200'
                }`}>
                  {post.published ? 'Published' : 'Draft'}
                </span>
              </div>
            ))}
            {!data?.recentBlog.length && <p className="text-sm text-gray-300">No posts yet.</p>}
          </div>
        </div>
      </div>
          ...
          <div className="flex gap-3 flex-wrap">
            {data.leadsByStatus.map((item) => (
              <div key={item.status} className="bg-gray-800 px-4 py-2 rounded-lg text-sm">
                <span className="text-gray-200 capitalize">{item.status}:</span>{' '}
