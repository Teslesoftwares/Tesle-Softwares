...
export default function ProjectDetail() {
  ...
  if (!project) return <div className="text-center py-12 text-gray-300">Project not found.</div>;
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-xs text-gray-200">Milestones</p>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        {!project.milestones?.length ? (
          <p className="text-sm text-gray-300">No milestones defined.</p>
        ) : (
            ...
            {project.milestones.map((m: any) => (
              <div key={m.id} className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className={`text-sm font-medium ${m.completed ? 'text-gray-200' : ''}`}>{m.title}</p>
                  {m.description && <p className="text-xs text-gray-300">{m.description}</p>}
